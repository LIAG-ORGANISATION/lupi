...
const DogsOrPatients = () => {
    ...
    try {
      if (role === "owner") {
        ...
        const {
          data,
          error
        } = await supabase.from("dogs" as any).select("*").order("name");
        ...
        const {
          data: sharesData,
          error: sharesError
        } = await supabase.from("dog_shares" as any).select("dog_id").eq("professional_id", (await supabase.auth.getUser()).data.user?.id).eq("status", "accepted");
        ...
        if (sharesData && sharesData.length > 0) {
          ...
          const {
            data: dogsData,
            error: dogsError
          } = await supabase.from("dogs" as any).select("*").in("id", dogIds).order("name");
  ...
  if (roleLoading || loading) {
    return <div className="min-h-screen flex items-center justify-center bg-background pb-24">
        <p className="text-muted-foreground">Chargement...</p>
        <BottomNavigation />
      </div>;
  }
  const pageTitle = role === "professional" ? "Mes patients" : "Mes chiens";
  const emptyMessage = role === "professional" ? "Aucun patient partagé avec vous pour le moment" : "Créez un profil pour votre compagnon";
  const emptySubtitle = role === "professional" ? "Les propriétaires pourront vous donner accès à leurs chiens" : "Suivez sa santé et son comportement";
  return <div className="min-h-screen p-4 space-y-6 animate-fade-in pb-24">
      ...
      {role === "owner" && <Card className="bg-secondary p-6 rounded-3xl space-y-4 shadow-md">
          <div>
      ...
      <div className="space-y-4">
        {dogs.length === 0 ? <Card className="p-8 rounded-3xl text-center shadow-md">
            <p className="text-muted-foreground">{emptyMessage}</p>
            {role === "professional" && <p className="text-sm text-muted-foreground mt-2">{emptySubtitle}</p>}
          </Card> : dogs.map(dog => <Card key={dog.id} className="p-4 rounded-3xl shadow-md hover:shadow-lg transition-all cursor-pointer" onClick={() => navigate(`/dogs/${dog.id}`)}>
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-secondary flex items-center justify-center text-2xl overflow-hidden">
                  {dog.avatar_url ? <img src={dog.avatar_url} alt={dog.name} className="w-full h-full object-cover" /> : "🐕"}
                </div>
                ...
                {role === "owner" && <Button variant="ghost" size="icon" onClick={e => {
            e.stopPropagation();
